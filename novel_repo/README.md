...
这**不是**一套"随便写点文案"的通用提示词，而是一套**可执行的长篇创作协议**——项目总览、角色档案、世界观、卷纲、章节、伏笔追踪、风格约束、一致性检查，全部纳入同一工作流。

如果这个项目对你有帮助，欢迎点一个 **Star** ⭐，帮助更多创作者发现它。
...
| 适合                                        | 不适合                      |
| ------------------------------------------- | --------------------------- |
| ✅ 正在写长篇网文（50 万字+）的作者         | ❌ 只想写短篇/文案的人      |
| ✅ 需要维护复杂人物关系和剧情线的创作者     | ❌ 不想维护项目结构的人     |
| ✅ 使用 AI 辅助创作，但被输出不一致困扰的人 | ❌ 期望一键生成完整小说的人 |
...
|     步骤      | 命令示例                                        |
| :-----------: | ----------------------------------------------- |
|  1️⃣ 创建项目  | `请用 novel-master 帮我创作一部长篇小说项目...` |
...
> 预期：生成 `00-overview.md` 项目总览和完整目录结构。
...
# 更新角色状态
第 15 章写完了，请更新林栖月的角色状态，记录她与镖局决裂后的新处境。
```

> 预期：生成/更新 `characters/林栖月.md`，同步到 `trackers/role-states.md`。

### 世界观构建

```text
请为"药行体系"生成世界观规则，每条规则必须包含：规则描述、限制条件、违反代价、破坏后的连锁后果。
```

> 预期：生成 `worldbuilding/药行体系.md`，规则具备可验证性。

### 卷纲与章节

```text
# 生成卷纲
请为第 2 卷生成大纲，卷主题是"逃亡与觉醒"，阶段目标是主角发现身世真相。

# 生成章节骨架
请为第 18 章生成骨架，必须包含开场锚点、核心冲突、关系变化点、认知变化点和章末钩子。

# 扩写正文
请根据第 18 章骨架扩写正文，控制在 2500-3000 字，保持林栖月的声线一致。
```

> 预期：生成 `volumes/vol-02.md` 卷纲和 `chapters/ch-018.md` 章节。

### 伏笔追踪

```text
# 埋设伏笔
请在第 12 章埋一个关于"师父验药笔记缺失页"的伏笔，暗示可能还活着。

# 检查伏笔状态
请检查当前所有伏笔的回收状态，列出已埋未收、已回收、跨卷呼应的情况。
```

> 预期：更新 `trackers/foreshadowing.md` 伏笔追踪表。
